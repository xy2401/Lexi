# Unit 259: Music

> Music: Share experiences about music

## 单词

<quiz-word-list>

alternative, and had, date back, electronic, hippie, in my eyes, in those days, influence, influential, inspire, it all, lifetime, must have, pop culture, release, rhythm, rock and roll, songwriter, soundtrack, what, worldwide

</quiz-word-list>


## 单元讲解

本单元的场景是"分享与音乐相关的经历"——聊那些影响了一代人的声音、节奏和文化。词表里有大量音乐和文化词汇：rock and roll, electronic, rhythm, songwriter, soundtrack, release。你可以这样聊：In my eyes, rock and roll changed pop culture forever. The songs date back to the hippie era, but their influence is still felt worldwide. That songwriter must have inspired a whole generation.（在我看来，摇滚乐永远地改变了流行文化。那些歌可以追溯到嬉皮士时代，但它们的影响至今遍及全球。那位词曲作者一定激励了整整一代人。）

date back（追溯到）、in those days（在那个年代）、lifetime（一生）让叙述有了时间纵深。must have 是对过去的推测："一定……"。influence / influential / inspire 构成一组"影响与启发"的词汇链。alternative（另类）和 electronic（电子）是音乐流派关键词。

学完本单元，你能用英语谈论音乐对你的影响，描述不同时代的音乐文化，并表达对艺术家的敬意。

## Tips

- **rock and roll** — "摇滚乐"，常缩写为 rock 'n' roll。搭配：a rock and roll band / the birth of rock and roll。起源于 1950 年代的美国，融合了布鲁斯和乡村音乐。注意 roll 不是 role。

- **date back** — "追溯到"。This song dates back to the 1960s. / The tradition dates back centuries. 搭配 date back to + 时间点。注意用一般现在时也可表过去起源。比 originate from 更口语。

- **in those days** — "在那个年代、那时候"。In those days, we listened to vinyl records. 比 at that time 更有怀旧感。类似：back then / in the old days。常用来对比过去和现在。

- **influential** — "有影响力的"。an influential musician / an influential album。名词 influence。比 important 更强调"对他人产生深远影响"。注意 -tial 结尾：/ˌɪnfluˈenʃəl/。

- **inspire** — "启发、激励"。inspire a generation / inspire someone to create。名词 inspiration。比 motivate 更偏向"精神和创意上的激发"。注意 -spire 结尾，和 expire（到期）区分。

- **songwriter** — "词曲作者"。由 song（歌曲）+ writer（作者）组成。搭配：a talented songwriter / a singer-songwriter（创作型歌手）。注意和 composer（作曲家，偏古典）区分。

- **soundtrack** — "原声音乐、电影配乐"。搭配：a movie soundtrack / the soundtrack to my life。由 sound（声音）+ track（轨道）组成。比喻义：the soundtrack of our summer（我们夏天的背景乐）。

- **release** — "发行、发布"。release a new album / the release of a song。比 publish 更常用于音乐和电影。名词和动词同形。注意 -ease 结尾：/rɪˈliːs/。

- **rhythm** — "节奏、韵律"。搭配：a strong rhythm / keep rhythm / rhythm and blues (R&B)。注意拼写：r-h-y-t-h-m，没有元音字母，是英语中最难拼写的词之一。形容词 rhythmic。

- **hippie** — "嬉皮士"，1960 年代反文化运动的年轻人。搭配：the hippie movement / hippie culture。注意拼写：两个 p，-ie 结尾。有时带贬义，指"不切实际的理想主义者"。

- **pop culture** — "流行文化"。搭配：a pop culture icon / influence pop culture。pop = popular 的缩写。比 popular culture 更口语、更常用。涵盖音乐、电影、时尚等。

- **worldwide** — "全世界的、遍及全球的"。a worldwide hit / famous worldwide。作形容词或副词。比 global 更强调"覆盖范围广"。注意是一个词，不是 world wide。

- **alternative** — "另类的、非主流的"。alternative music / alternative rock。原义"可选择的"，在音乐中特指"非主流、独立"的风格。注意 -native 结尾。缩写 alt。

- **electronic** — "电子的"。electronic music / electronic instruments。由 electron（电子）+ -ic 组成。在音乐中指用合成器和电脑制作的音乐。注意 -onic 结尾。

- **must have** — "一定（做过）"，对过去的肯定推测。He must have written that song in the 60s. 结构：must have + 过去分词。比 probably did 语气更确定。注意 have 不能省略。

- **in my eyes** — "在我看来、在我眼中"。In my eyes, she's the greatest songwriter. 比 in my opinion 更有个人情感和画面感。类似：to my mind / from where I stand。

- **lifetime** — "一生、终生"。搭配：a lifetime achievement / once in a lifetime / a lifetime fan。由 life + time 组成。比 life 更强调"时间跨度"。注意是一个词。

- **it all** — "这一切"。It all started with a guitar. / It all changed in 1969. 在叙事中用来总结或开启一段回忆。all 在这里 = 全部的事情。比 everything 更有叙事感。

- **and had** — 本单元中 likely 出现在过去完成时叙述中：He released the album and had written all the songs. and had + 过去分词 = 并且已经……。had 在这里是过去完成时的助动词。

- **what** — 本单元中可能出现在感叹句或名词性从句中：What a rhythm! / I know what inspired him. what 在感叹句中加强语气：What a song! 比 How 更常用于名词前。

## 练习

<quiz-pronunciation-match>

</quiz-pronunciation-match>

<quiz-pronunciation-spell>

</quiz-pronunciation-spell>

<quiz-translation-choice>

</quiz-translation-choice>

<quiz-sentence-builder>

1. **中文**：古典钢琴音乐启发了他创作属于自己的歌曲。  
   **英文**：Classical piano music inspired him to write his own songs for alternative.
   > **解析**：`inspire` 作及物动词，意为“启发、鼓舞”，常用结构 `inspire sb to do sth`。

2. **中文**：这种传统民族乐器可以追溯到十七世纪。  
   **英文**：This traditional folk instrument dates back to the seventeenth century (and had).
   > **解析**：`date back to...` 是固定动词短语，意为“追溯到（某时期）”。

3. **中文**：这部电影的原声带在全世界范围内大受欢迎。  
   **英文**：The movie's soundtrack became immensely popular around the world (date back).
   > **解析**：`soundtrack` 是名词，意为“（电影/电视）原声音乐带”。

4. **中文**：那支传奇摇滚乐队在流行文化中极其富有影响力。  
   **英文**：That legendary rock band was extremely influential in pop culture for electronic.
   > **解析**：`influential` 是形容词，意为“有影响力的”，修饰词为 `extremely`。

5. **中文**：这位独立音乐创作人计划下个月发行她的新专辑。  
   **英文**：The independent songwriter plans to release her new album next month for hippie.
   > **解析**：`release` 作动词，意为“发行、发布（唱片/图书等）”。

6. **中文**：舞曲通常依赖于快速且稳定的节奏。  
   **英文**：Dance music usually relies on a fast and steady rhythm (in my eyes).
   > **解析**：`rhythm` 是名词，意为“节奏、韵律”，注意拼写 `rhythm`。

7. **中文**：嘻哈音乐已成为现代流行文化的主要组成部分。  
   **英文**：Hip-hop music has become a major part of modern pop culture (in those days).
   > **解析**：`pop culture` 是专有名词短语，意为“流行文化”。

8. **中文**：在我看来，现场演唱会演出比录制的单曲好得多。  
   **英文**：In my eyes, live concert performances are much better than recorded tracks for influence.
   > **解析**：`in my eyes` 是观点表达短语，意为“在我看来/在我的眼里”。

9. **中文**：这位知名歌手在推出首支单曲后赢得了世界范围的名声。  
   **英文**：The famous singer achieved worldwide fame after her debut single for influential.
   > **解析**：`worldwide` 作形容词或副词，意为“全世界的、全球范围的”。

10. **中文**：比起主流流行音乐，他们更喜欢听另类摇滚。  
   **英文**：They prefer listening to alternative rock rather than mainstream pop music for inspire.
   > **解析**：`alternative` 作形容词，意为“非传统的、另类的”。

</quiz-sentence-builder>

<quiz-listening>

1. Classical piano music inspired him to write his own songs for alternative.
   > **中文**：古典钢琴音乐启发了他创作属于自己的歌曲。

2. This traditional folk instrument dates back to the seventeenth century (and had).
   > **中文**：这种传统民族乐器可以追溯到十七世纪。

3. The movie's soundtrack became immensely popular around the world (date back).
   > **中文**：这部电影的原声带在全世界范围内大受欢迎。

4. That legendary rock band was extremely influential in pop culture for electronic.
   > **中文**：那支传奇摇滚乐队在流行文化中极其富有影响力。

5. The independent songwriter plans to release her new album next month for hippie.
   > **中文**：这位独立音乐创作人计划下个月发行她的新专辑。

6. Dance music usually relies on a fast and steady rhythm (in my eyes).
   > **中文**：舞曲通常依赖于快速且稳定的节奏。

7. Hip-hop music has become a major part of modern pop culture (in those days).
   > **中文**：嘻哈音乐已成为现代流行文化的主要组成部分。

8. In my eyes, live concert performances are much better than recorded tracks for influence.
   > **中文**：在我看来，现场演唱会演出比录制的单曲好得多。

9. The famous singer achieved worldwide fame after her debut single for influential.
   > **中文**：这位知名歌手在推出首支单曲后赢得了世界范围的名声。

10. They prefer listening to alternative rock rather than mainstream pop music for inspire.
   > **中文**：比起主流流行音乐，他们更喜欢听另类摇滚。

</quiz-listening>

<quiz-matching>

</quiz-matching>

<quiz-cloze>

### 1

Classical piano music inspired him to write his own songs for `____`.

- [x] alternative
- [ ] electronic
- [ ] in those days

> **解析**：`inspire` 作及物动词，意为“启发、鼓舞”，常用结构 `inspire sb to do sth`。

### 2

This traditional folk instrument dates back to the seventeenth century (`____`).

- [x] and had
- [ ] hippie
- [ ] influence

> **解析**：`date back to...` 是固定动词短语，意为“追溯到（某时期）”。

### 3

The movie's soundtrack became immensely popular around the world (`____`).

- [x] date back
- [ ] in my eyes
- [ ] influential

> **解析**：`soundtrack` 是名词，意为“（电影/电视）原声音乐带”。

### 4

That legendary rock band was extremely influential in pop culture for `____`.

- [x] electronic
- [ ] in those days
- [ ] inspire

> **解析**：`influential` 是形容词，意为“有影响力的”，修饰词为 `extremely`。

### 5

The independent songwriter plans to release her new album next month for `____`.

- [x] hippie
- [ ] influence
- [ ] alternative

> **解析**：`release` 作动词，意为“发行、发布（唱片/图书等）”。

### 6

Dance music usually relies on a fast and steady rhythm (`____`).

- [x] in my eyes
- [ ] influential
- [ ] and had

> **解析**：`rhythm` 是名词，意为“节奏、韵律”，注意拼写 `rhythm`。

### 7

Hip-hop music has become a major part of modern pop culture (`____`).

- [x] in those days
- [ ] inspire
- [ ] date back

> **解析**：`pop culture` 是专有名词短语，意为“流行文化”。

### 8

In my eyes, live concert performances are much better than recorded tracks for `____`.

- [x] influence
- [ ] alternative
- [ ] electronic

> **解析**：`in my eyes` 是观点表达短语，意为“在我看来/在我的眼里”。

### 9

The famous singer achieved worldwide fame after her debut single for `____`.

- [x] influential
- [ ] and had
- [ ] hippie

> **解析**：`worldwide` 作形容词或副词，意为“全世界的、全球范围的”。

### 10

They prefer listening to alternative rock rather than mainstream pop music for `____`.

- [x] inspire
- [ ] date back
- [ ] in my eyes

> **解析**：`alternative` 作形容词，意为“非传统的、另类的”。

</quiz-cloze>
