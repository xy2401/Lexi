# Unit 226: Reunion 2

> Reunion: Share surprising updates

## 单词

<quiz-word-list>

ages, back then, bond, campus, find out, fluent, get, get over, happy hour, influence, look back on, make a fool, neither of, related, stand out, take after, taste, the, turn out, who, whoever, whose

</quiz-word-list>


## 单元讲解

本单元围绕"分享令人惊讶的近况"展开，场景是老友重逢或同学聚会。词表里有回忆与时间表达（ages, back then, look back on），有人际关系词汇（bond, influence, take after, related, neither of），也有"发现与结果"的表达（find out, turn out, stand out）。你可以用这些词聊重逢的惊喜：It's been ages! / Back then, neither of us was fluent in English. / I found out that she's a professor now—who would've thought? / It turns out we have more in common than I thought. / Looking back on our campus days, we really had a strong bond.

这里值得注意 turn out 和 find out 的区别：find out 强调"发现"（主动获取信息），turn out 强调"结果是"（事情的发展出乎意料）。另外，take after 是一个有趣的短语动词——它表示"长得像/性格像（长辈）"：She takes after her mother.

学完本单元，你可以用英语在重逢场景中分享近况、回忆过去、表达惊讶，并谈论人与人之间的相似和影响。

## Tips

- **turn out / find out** — turn out = 结果是（强调意外）：It turned out he was right. find out = 发现、查明（强调获取信息）：I found out the truth. 区别：turn out 主语常是事情，find out 主语常是人。

- **take after** — （外貌或性格）像（长辈）。搭配：take after sb, take after your mother/father。She takes after her dad—same smile. 注意 take after 只用于家庭成员间的相似，不用于朋友。

- **look back on** — 回顾、回想。搭配：look back on the past, look back on our school days。Looking back on campus life, I feel nostalgic. 比 remember 更有"回望过去"的画面感。

- **ages** — 很久（口语夸张说法）。搭配：It's been ages, for ages, ages ago。It's been ages since we met! / I haven't seen you in ages. 注意 ages 是复数，表"好多年"，实际可能只是几个月。

- **back then** — 那时候、当时。搭配：back then, back in those days。Back then, we were just students. 比 at that time 更口语化，带有怀旧感。常放在句首。

- **bond** — 纽带、情感联系。搭配：a strong bond, bond with sb, form a bond。We bonded over our love of music. 可作名词也可作动词。注意 bond 比 friendship 更强调深层的情感连接。

- **fluent** — 流利的（语言）。搭配：fluent in English, speak fluent French。She's fluent in three languages. 注意搭配介词 in：fluent in（不是 × fluent of）。名词是 fluency。

- **stand out** — 突出、显眼。搭配：stand out from the crowd, stand out in a group。She really stood out on campus. 注意 stand out 是不及物动词短语，不接宾语（× stand out others）。

- **make a fool** — 出丑、闹笑话。完整搭配：make a fool of yourself, make a fool of sb。I made a fool of myself at happy hour. 注意 make a fool of yourself = 让自己出丑。

- **neither of** — 两者都不。搭配：neither of us/them, neither of the two。Neither of us was ready. 注意 neither 后动词用单数（正式）或复数（口语均可）：Neither of them is/are here.

- **whose / whoever** — whose 是"谁的"（所有格关系代词），whoever 是"无论是谁"。搭配：whose idea was it, whoever said that。Whose turn is it? / Whoever wins, I'll be happy. 注意 whose 可引导定语从句：the person whose car was stolen。

- **influence / related** — influence 是影响（可作名词和动词），related 是相关的/有亲属关系的。搭配：have an influence on, be related to。She influenced my taste in music. / Are you two related? 注意 related 既可指"有关联"也可指"有亲戚关系"。

- **happy hour** — 欢乐时光（酒吧/餐厅的打折时段，通常下午5-7点）。搭配：go to happy hour, happy hour drinks。Let's meet at happy hour. 这是北美社交文化中的常见概念。

## 练习

<quiz-pronunciation-match>

</quiz-pronunciation-match>

<quiz-pronunciation-spell>

</quiz-pronunciation-spell>

<quiz-translation-choice>

</quiz-translation-choice>

<quiz-sentence-builder>

1. **中文**：我好久没有见到我的大学同学了。  
   **英文**：I haven't seen my college classmates for ages.
   > **解析**：for ages 表示很久、许久。

2. **中文**：那时候我们常常一聊就是几个小时。  
   **英文**：We used to spend hours talking back then.
   > **解析**：back then 表示那时候。

3. **中文**：我们昨天在欢乐时光打折时段于酒吧相聚。  
   **英文**：We met at the bar during happy hour yesterday.
   > **解析**：happy hour 指（酒吧等的）欢乐时光打折时段。

4. **中文**：走在大学校园里勾起了过去的回忆。  
   **英文**：Walking around the university campus brought back old memories.
   > **解析**：campus 指校园。

5. **中文**：他们在读书期间建立了深厚的纽带。  
   **英文**：They formed a strong bond during their school years.
   > **解析**：bond 指感情纽带、联系。

6. **中文**：当我回顾我的年轻时光，我充满感激。  
   **英文**：When I look back on my youth, I feel grateful.
   > **解析**：look back on 表示回顾、回首。

7. **中文**：我惊讶地发现我们就住在附近。  
   **英文**：I was surprised to find out that we live nearby.
   > **解析**：find out 表示发现、打听到。

8. **中文**：结果发现他是我的表弟的好朋友。  
   **英文**：It turned out that he is my cousin's good friend.
   > **解析**：turn out 表示结果发现是……

9. **中文**：她花了很长时间才走出了失落。  
   **英文**：It took her a long time to get over the loss.
   > **解析**：get over 表示克服、走出（阴影）。

10. **中文**：不要在家庭聚会上出糗。  
   **英文**：Don't make a fool of yourself at the family party.
   > **解析**：make a fool of oneself 表示出糗、扮蠢。

11. **中文**：她的红裙子使她在人群中脱颖而出。  
   **英文**：Her red dress made her stand out in the crowd.
   > **解析**：stand out 表示出众、显眼。

12. **中文**：他的性格真的很像他的祖父。  
   **英文**：He really takes after his grandfather in personality.
   > **解析**：take after 表示（长相或性格）像（长辈）。

13. **中文**：他们俩都不记得聚会的确切日期了。  
   **英文**：Neither of them remembered the exact date of the party.
   > **解析**：neither of... 表示“两者都不……”。

14. **中文**：她现在精通三门外语。  
   **英文**：She is fluent in three foreign languages now.
   > **解析**：fluent 表示流利的、精通的。

15. **中文**：我想知道他们之间是否有亲戚关系。  
   **英文**：I wonder if they are related to each other.
   > **解析**：related 表示有亲属关系的。

16. **中文**：我祖母对我职业选择有着巨大影响。  
   **英文**：My grandmother had a great influence on my career choice.
   > **解析**：influence 表示影响。

17. **中文**：我们在传统音乐方面有着相似的品味。  
   **英文**：We share a similar taste in traditional music.
   > **解析**：taste 表示品味。

18. **中文**：你知道谁参加了同学聚会吗？  
   **英文**：Do you know who came to the reunion party?
   > **解析**：who 疑问代词表示“谁”。

19. **中文**：无论是谁先到都应该把大门打开。  
   **英文**：Whoever arrives first should open the entrance door.
   > **解析**：whoever 表示无论是谁。

20. **中文**：猜猜谁的照片展示在了公告栏上？  
   **英文**：Guess whose photo was displayed on the bulletin board?
   > **解析**：whose 表示“谁的”。

</quiz-sentence-builder>

<quiz-listening>

1. I haven't seen my college classmates for ages.
   > **中文**：我好久没有见到我的大学同学了。

2. We used to spend hours talking back then.
   > **中文**：那时候我们常常一聊就是几个小时。

3. We met at the bar during happy hour yesterday.
   > **中文**：我们昨天在欢乐时光打折时段于酒吧相聚。

4. Walking around the university campus brought back old memories.
   > **中文**：走在大学校园里勾起了过去的回忆。

5. They formed a strong bond during their school years.
   > **中文**：他们在读书期间建立了深厚的纽带。

6. When I look back on my youth, I feel grateful.
   > **中文**：当我回顾我的年轻时光，我充满感激。

7. I was surprised to find out that we live nearby.
   > **中文**：我惊讶地发现我们就住在附近。

8. It turned out that he is my cousin's good friend.
   > **中文**：结果发现他是我的表弟的好朋友。

9. It took her a long time to get over the loss.
   > **中文**：她花了很长时间才走出了失落。

10. Don't make a fool of yourself at the family party.
   > **中文**：不要在家庭聚会上出糗。

11. Her red dress made her stand out in the crowd.
   > **中文**：她的红裙子使她在人群中脱颖而出。

12. He really takes after his grandfather in personality.
   > **中文**：他的性格真的很像他的祖父。

13. Neither of them remembered the exact date of the party.
   > **中文**：他们俩都不记得聚会的确切日期了。

14. She is fluent in three foreign languages now.
   > **中文**：她现在精通三门外语。

15. I wonder if they are related to each other.
   > **中文**：我想知道他们之间是否有亲戚关系。

16. My grandmother had a great influence on my career choice.
   > **中文**：我祖母对我职业选择有着巨大影响。

17. We share a similar taste in traditional music.
   > **中文**：我们在传统音乐方面有着相似的品味。

18. Do you know who came to the reunion party?
   > **中文**：你知道谁参加了同学聚会吗？

19. Whoever arrives first should open the entrance door.
   > **中文**：无论是谁先到都应该把大门打开。

20. Guess whose photo was displayed on the bulletin board?
   > **中文**：猜猜谁的照片展示在了公告栏上？

</quiz-listening>

<quiz-matching>

| 英文单词 | 课文释义 |
| :--- | :--- |
| `ages` | 很长时间 |
| `back then` | 那时候 |
| `bond` | 纽带；联系 |
| `campus` | 校园 |
| `find out` | 发现；查明 |
| `fluent` | 流利的 |
| `get over` | 走出（阴影）；克服 |
| `happy hour` | 欢乐时光打折时段 |
| `influence` | 影响 |
| `look back on` | 回顾；回首 |
| `make a fool` | 出糗 |
| `neither of` | 两者都不 |
| `related` | 有亲属关系的 |
| `stand out` | 脱颖而出；显眼 |
| `take after` | 长得像（长辈） |
| `taste` | 品味 |
| `turn out` | 结果发现是 |
| `who` | 谁 |
| `whoever` | 无论是谁 |
| `whose` | 谁的 |

</quiz-matching>

<quiz-cloze>

### 1

I haven't seen my college classmates for ____.

- [x] ages
- [ ] back then
- [ ] bond

> **解析**：主语为第三人称单数，谓语动词使用单三形式 **ages**。

### 2

We used to spend hours ____ back then.

- [x] talking
- [ ] ages
- [ ] back then

> **解析**：**talking** 为动词的现在分词/动名词形式，配合句中语法结构。

### 3

We met at ____ bar during happy hour yesterday.

- [x] the
- [ ] ages
- [ ] back then

> **解析**：**the** 为定冠词，用于特指双方已知的人或事物。

### 4

Walking around ____ university campus brought back old memories.

- [x] the
- [ ] ages
- [ ] back then

> **解析**：**the** 为定冠词，用于特指双方已知的人或事物。

### 5

They formed a strong ____ during their school years.

- [x] bond
- [ ] ages
- [ ] back then

> **解析**：**bond** 符合句意表达，作为句中的核心词汇补全句子。

### 6

When I look back on my youth, I feel ____.

- [x] grateful
- [ ] ages
- [ ] back then

> **解析**：**grateful** 符合句意表达，作为句中的核心词汇补全句子。

### 7

I was ____ to find out that we live nearby.

- [x] surprised
- [ ] ages
- [ ] back then

> **解析**：句中描述过去发生的事情，谓语动词使用过去式 **surprised**。

### 8

It turned out that he is my ____ good friend.

- [x] cousin's
- [ ] ages
- [ ] back then

> **解析**：主语为第三人称单数，谓语动词使用单三形式 **cousin's**。

### 9

It took her a long time to ____ over the loss.

- [x] get
- [ ] ages
- [ ] back then

> **解析**：**get** 符合句意表达，作为句中的核心词汇补全句子。

### 10

Don't make a fool of yourself at ____ family party.

- [x] the
- [ ] ages
- [ ] back then

> **解析**：**the** 为定冠词，用于特指双方已知的人或事物。

### 11

Her red dress made her stand out in ____ crowd.

- [x] the
- [ ] ages
- [ ] back then

> **解析**：**the** 为定冠词，用于特指双方已知的人或事物。

### 12

He really takes after his ____ in personality.

- [x] grandfather
- [ ] ages
- [ ] back then

> **解析**：**grandfather** 为形容词/副词比较级，用于两者之间的对比。

### 13

Neither of them remembered ____ exact date of the party.

- [x] the
- [ ] ages
- [ ] back then

> **解析**：**the** 为定冠词，用于特指双方已知的人或事物。

### 14

She is ____ in three foreign languages now.

- [x] fluent
- [ ] ages
- [ ] back then

> **解析**：**fluent** 符合句意表达，作为句中的核心词汇补全句子。

### 15

I wonder if they are ____ to each other.

- [x] related
- [ ] ages
- [ ] back then

> **解析**：句中描述过去发生的事情，谓语动词使用过去式 **related**。

### 16

My grandmother had a great ____ on my career choice.

- [x] influence
- [ ] ages
- [ ] back then

> **解析**：**influence** 符合句意表达，作为句中的核心词汇补全句子。

### 17

We share a similar ____ in traditional music.

- [x] taste
- [ ] ages
- [ ] back then

> **解析**：**taste** 符合句意表达，作为句中的核心词汇补全句子。

### 18

Do you know ____ came to the reunion party?

- [x] who
- [ ] ages
- [ ] back then

> **解析**：**who** 符合句意表达，作为句中的核心词汇补全句子。

### 19

____ arrives first should open the entrance door.

- [x] Whoever
- [ ] ages
- [ ] back then

> **解析**：**Whoever** 为形容词/副词比较级，用于两者之间的对比。

### 20

Guess ____ photo was displayed on the bulletin board?

- [x] whose
- [ ] ages
- [ ] back then

> **解析**：**whose** 符合句意表达，作为句中的核心词汇补全句子。

</quiz-cloze>