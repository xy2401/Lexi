# Unit 266: Detective

> Detective: Describe suspicious events

## 单词

<quiz-word-list>

as, based on, bullet, case, clue, considering, could have, crucial, disguise, distract, fingerprint, footprint, gang, go off, it could have been, it must have been, justice, may have, may not have, motive, must have, robber, suspect, there may have been, weapon

</quiz-word-list>


## 单元讲解

本单元的场景是"描述可疑事件"——一场侦探推理，有线索、嫌疑人、动机和证据。词表里有大量刑侦词汇：clue, fingerprint, footprint, motive, suspect, weapon, bullet。你可以这样推理：Based on the fingerprints and footprints, it must have been someone in the gang. There may have been a disguise involved. Considering the motive, it could have been the robber—but we need more clues to crack the case.（根据指纹和脚印，一定是团伙里的人。可能涉及伪装。考虑到动机，可能是那个劫匪——但我们需要更多线索来破案。）

must have / could have / may have 是对过去事件的三种推测，确定性依次递减。it must have been（一定是）、it could have been（可能是）、there may have been（也许有）让推理层次分明。go off（响起/离开）可能指警报或枪声。justice（正义）是侦探故事的终极目标。

学完本单元，你能用英语进行逻辑推理，描述犯罪现场，评估证据，并表达对事件的不同确定程度。

## Tips

- **must have / could have / may have** — 对过去推测的三种情态动词。must have = 一定是（最确定）；could have = 可能是（有可能）；may have = 也许（最不确定）。结构都是：情态动词 + have + 过去分词。

- **it must have been** — "一定是（某人/某物）"。It must have been the robber. 比 it was probably 语气强得多，表示"根据证据几乎可以确定"。注意 must have been 不能缩写为 must've been（正式写作中）。

- **it could have been** — "可能是、本来是"。It could have been anyone in the gang. 比 must have 不确定，暗示"有这种可能性"。也可以表虚拟：It could have been worse.（本来可能更糟。）

- **there may have been** — "也许有、可能存在过"。There may have been an accomplice. 比 there was probably 更不确定、更谨慎。may have 在正式推理中比 might have 更常见。

- **based on** — "基于、根据"。Based on the evidence, he's guilty. 比 according to 更强调"以……为基础进行推断"。注意 based on 是过去分词短语，常放句首。

- **considering** — "考虑到、鉴于"。Considering the motive, she's the prime suspect. 比 given 更口语。作介词用，后接名词或 that 从句。注意 -ing 形式。

- **crucial** — "关键的、至关重要的"。a crucial clue / a crucial piece of evidence。比 important 程度强得多，暗示"缺了它就不行"。名词 cruciality（较少用）。注意 -cial 结尾：/ˈkruːʃəl/。

- **motive** — "动机"。搭配：a motive for murder / What was the motive? / no apparent motive。比 reason 更专指"犯罪或行为的内在驱动力"。注意 -tive 结尾：/ˈmoʊtɪv/。

- **suspect** — "嫌疑人；怀疑"。搭配：a prime suspect / suspect someone of a crime。注意名词重音在前 /ˈsʌspekt/，动词重音在后 /səˈspekt/。形容词 suspicious。

- **disguise** — "伪装、 disguises"。搭配：in disguise / a clever disguise / disguise yourself。动词：He disguised himself as a guard。注意 -guise 结尾：/dɪsˈɡaɪz/。比 costume 更强调"隐藏身份"。

- **distract** — "分散注意力、使分心"。distract the guard / a distraction。搭配 distract someone from something。比 interrupt 更强调"把注意力引开"。名词 distraction。注意 -tract 结尾。

- **fingerprint** — "指纹"。搭配：leave fingerprints / fingerprint evidence / take someone's fingerprints。由 finger（手指）+ print（印记）组成。动词：The police fingerprinted him。

- **footprint** — "脚印、足迹"。搭配：a footprint in the mud / follow the footprints。由 foot（脚）+ print（印记）组成。比喻义：a carbon footprint（碳足迹）。注意复数 footprints。

- **bullet** — "子弹"。搭配：a bullet wound / a stray bullet / dodge a bullet。比 ammunition（弹药）更具体，指"单颗子弹"。注意双 l：bul-let。比喻义：dodge a bullet = 躲过一劫。

- **weapon** — "武器"。搭配：a murder weapon / a deadly weapon / weapon of choice。比 gun 更广义，包括刀、棍等。注意 -pon 结尾：/ˈwepən/。形容词 weaponless（无武器的）。

- **gang** — "团伙、帮派"。搭配：a gang of robbers / gang member / a street gang。比 group 更有"犯罪组织"的意味。注意 -ng 结尾。动词 gang up on = 联合对付。

- **robber** — "劫匪、强盗"。搭配：a bank robber / an armed robber。动词 rob，名词 robbery（抢劫案）。注意 -er 表人。和 thief（小偷）区分：robber 通常指"用暴力或威胁抢劫"。

- **clue** — "线索"。搭配：a vital clue / follow a clue / not a clue。比 hint 更具体，指"帮助破案的证据或信息"。注意 -ue 结尾。口语中 have no clue = 完全不知道。

- **case** — "案件；箱子"。搭配：a murder case / crack the case / case closed。在法律和刑侦语境中 = 案件。一词多义：a case of wine（一箱酒）。注意 -se 结尾：/keɪs/。

- **justice** — "正义、司法"。搭配：bring someone to justice / justice system / social justice。形容词 just（公正的）。注意 -tice 结尾：/ˈdʒʌstɪs/。比 fairness 更正式、更制度化。

- **go off** — 一词多义：① 响起（The alarm went off.）② 爆炸（The bomb went off.）③ 离开（He went off to investigate.）。在侦探语境中，go off 常指警报或枪声。非常灵活的短语动词。

- **as** — 本单元中 likely 指"作为"或"当……时"。as a detective / as the suspect fled。as 是英语中最灵活的词之一：作为、当、因为、如同。具体含义高度依赖语境。

## 练习

<quiz-pronunciation-match>

</quiz-pronunciation-match>

<quiz-pronunciation-spell>

</quiz-pronunciation-spell>

<quiz-translation-choice>

</quiz-translation-choice>

<quiz-sentence-builder>

1. **中文**：侦探仔细检查了房间，以寻找任何隐蔽的线索。  
   **英文**：The detective examined the room carefully to find any hidden clues.
   > **解析**：`clue` 是可数名词，意为“线索、提示”，常用搭配 `find a clue`。

2. **中文**：法医专家从未锁的玻璃门上收集了指纹。  
   **英文**：Forensic experts gathered fingerprints from the unlocked glass door (based on).
   > **解析**：`fingerprint` 是名词，意为“指纹”，常用复数 `fingerprints`。

3. **中文**：警方今天下午带回了主要犯罪嫌疑人进行审问。  
   **英文**：Police brought the prime suspect in for questioning this afternoon.
   > **解析**：`suspect` 作名词时意为“犯罪嫌疑人”，作宾语。

4. **中文**：看着强行破坏入侵的痕迹，那一定是一个经验丰富的强盗。  
   **英文**：Looking at the forced entry, it must have been an experienced robber.
   > **解析**：`it must have been...` 表示对过去事实的推测：“那一定曾经是……”。

5. **中文**：调查人员正在寻找这次突然盗窃案背后的明确动机。  
   **英文**：Investigators are searching for a clear motive behind the sudden theft.
   > **解析**：`motive` 是名词，意为“动机、目的”，短语 `motive behind sth`。

6. **中文**：找到丢失的证据对破解这桩疑案至关重要。  
   **英文**：Finding the missing evidence is crucial to solving the mystery case.
   > **解析**：`crucial` 是形容词，意为“至关重要的、决定性的”，作表语。

7. **中文**：入侵者穿戴了伪装，以避免在摄像头中被认出。  
   **英文**：The intruder wore a disguise to avoid being recognized on camera (could have).
   > **解析**：`disguise` 是名词，意为“伪装、假扮”，搭配 `wear a disguise`。

8. **中文**：根据初步调查结果，盗窃案发生在午夜过后的某个时间。  
   **英文**：Based on initial findings, the robbery occurred sometime after midnight.
   > **解析**：`based on...` 是介词短语，意为“根据……、基于……”，引导状语。

9. **中文**：侦探证实犯罪现场没有发现任何武器。  
   **英文**：The detective confirmed that no weapon was found at the crime scene.
   > **解析**：`weapon` 是名词，意为“武器、凶器”。

10. **中文**：作案者本来有可能通过地下室窗户逃脱了。  
   **英文**：The perpetrator could have escaped through the basement window about bullet.
   > **解析**：`could have + 过去分词` 表示对过去可能性的推测（本来可能）。

</quiz-sentence-builder>

<quiz-listening>

1. The detective examined the room carefully to find any hidden clues.
   > **中文**：侦探仔细检查了房间，以寻找任何隐蔽的线索。

2. Forensic experts gathered fingerprints from the unlocked glass door (based on).
   > **中文**：法医专家从未锁的玻璃门上收集了指纹。

3. Police brought the prime suspect in for questioning this afternoon.
   > **中文**：警方今天下午带回了主要犯罪嫌疑人进行审问。

4. Looking at the forced entry, it must have been an experienced robber.
   > **中文**：看着强行破坏入侵的痕迹，那一定是一个经验丰富的强盗。

5. Investigators are searching for a clear motive behind the sudden theft.
   > **中文**：调查人员正在寻找这次突然盗窃案背后的明确动机。

6. Finding the missing evidence is crucial to solving the mystery case.
   > **中文**：找到丢失的证据对破解这桩疑案至关重要。

7. The intruder wore a disguise to avoid being recognized on camera (could have).
   > **中文**：入侵者穿戴了伪装，以避免在摄像头中被认出。

8. Based on initial findings, the robbery occurred sometime after midnight.
   > **中文**：根据初步调查结果，盗窃案发生在午夜过后的某个时间。

9. The detective confirmed that no weapon was found at the crime scene.
   > **中文**：侦探证实犯罪现场没有发现任何武器。

10. The perpetrator could have escaped through the basement window about bullet.
   > **中文**：作案者本来有可能通过地下室窗户逃脱了。

</quiz-listening>

<quiz-matching>

</quiz-matching>

<quiz-cloze>

### 1

The detective examined the room carefully to find any hidden `____`s.

- [x] clue
- [ ] as
- [ ] considering

> **解析**：`clue` 是可数名词，意为“线索、提示”，常用搭配 `find a clue`。

### 2

Forensic experts gathered fingerprints from the unlocked gl`____`s door (based on).

- [x] as
- [ ] bullet
- [ ] could have

> **解析**：`fingerprint` 是名词，意为“指纹”，常用复数 `fingerprints`。

### 3

Police brought the prime `____` in for questioning this afternoon.

- [x] suspect
- [ ] bullet
- [ ] could have

> **解析**：`suspect` 作名词时意为“犯罪嫌疑人”，作宾语。

### 4

Looking at the forced entry, `____` an experienced robber.

- [x] it must have been
- [ ] case
- [ ] crucial

> **解析**：`it must have been...` 表示对过去事实的推测：“那一定曾经是……”。

### 5

Investigators are searching for a clear `____` behind the sudden theft.

- [x] motive
- [ ] clue
- [ ] disguise

> **解析**：`motive` 是名词，意为“动机、目的”，短语 `motive behind sth`。

### 6

Finding the missing evidence is crucial to solving the mystery `____`.

- [x] case
- [ ] could have
- [ ] fingerprint

> **解析**：`crucial` 是形容词，意为“至关重要的、决定性的”，作表语。

### 7

The intruder wore a disguise to avoid being recognized on camera (`____`).

- [x] could have
- [ ] crucial
- [ ] footprint

> **解析**：`disguise` 是名词，意为“伪装、假扮”，搭配 `wear a disguise`。

### 8

`____` initial findings, the robbery occurred sometime after midnight.

- [x] based on
- [ ] disguise
- [ ] gang

> **解析**：`based on...` 是介词短语，意为“根据……、基于……”，引导状语。

### 9

The detective confirmed that no `____` was found at the crime scene.

- [x] weapon
- [ ] disguise
- [ ] gang

> **解析**：`weapon` 是名词，意为“武器、凶器”。

### 10

The perpetrator could have escaped through the basement window about `____`.

- [x] bullet
- [ ] fingerprint
- [ ] it could have been

> **解析**：`could have + 过去分词` 表示对过去可能性的推测（本来可能）。

</quiz-cloze>
