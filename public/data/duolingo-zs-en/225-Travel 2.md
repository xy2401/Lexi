# Unit 225: Travel 2

> Travel: Describe ideal vacation spots

## 单词

<quiz-word-list>

backup plan, deck, diner, get away, hike, in the middle of nowhere, itinerary, just in case, more, not only, off the coast of, outskirts, sold out, spontaneous, stop, stormy, sunburn, take, than, tropical, unpredictable, volcano

</quiz-word-list>


## 单元讲解

本单元围绕"描述理想的度假地点"展开。词表里有目的地与场景词汇（tropical, volcano, off the coast of, outskirts, in the middle of nowhere, diner, deck），也有旅行规划与意外（itinerary, backup plan, just in case, sold out, spontaneous, unpredictable, stormy）。你可以用这些词讨论旅行计划：I want to get away to a tropical island off the coast of somewhere. / Not only is it beautiful, but there's a volcano you can hike. / The weather is unpredictable and stormy, so we need a backup plan just in case. / The hotel is sold out—let's be spontaneous and find a diner on the outskirts.

这里值得注意 not only...but (also) 的递进结构，以及 in the middle of nowhere 这个生动的习语——它形容一个极其偏僻、前不着村后不着店的地方。spontaneous 和 itinerary 形成有趣的对比：一个随性，一个按计划。

学完本单元，你可以用英语描绘理想度假地、讨论旅行中的变数，并在计划与随性之间表达偏好。

## Tips

- **in the middle of nowhere** — 前不着村后不着店、极其偏僻的地方。搭配：in the middle of nowhere, out in the middle of nowhere。The diner was in the middle of nowhere. 这是一个常用口语表达，带点夸张，不一定真的荒无人烟。

- **itinerary / backup plan / just in case** — itinerary 是行程表，backup plan 是备用方案，just in case 是"以防万一"。搭配：plan an itinerary, have a backup plan, take an umbrella just in case。I made an itinerary, but I also have a backup plan just in case things go wrong.

- **spontaneous** — 自发的、随性的、说走就走的。搭配：a spontaneous trip, be spontaneous。Let's be spontaneous—no itinerary! 反义词是 planned。注意发音：/spɑːnˈteɪniəs/。

- **off the coast of** — 在……海岸外（的海上）。搭配：an island off the coast of Italy, off the coast of Africa。There's a tropical island off the coast. 注意 off 表示"离开海岸一段距离"，不是 on the coast（在海岸上）。

- **sold out** — 售罄、满员。搭配：be sold out, the tickets are sold out。The hotel is sold out for the weekend. 注意 sold out 是形容词性短语：It's sold out.（× It's sold-out. 作表语时不加连字符）

- **get away** — 逃离、去度假。搭配：get away from it all, a weekend get-away。I need to get away for a few days. 名词形式 getaway（连写）= 短途度假。注意 get away 强调"离开日常"。

- **unpredictable / stormy** — unpredictable 是不可预测的，stormy 是暴风雨的。搭配：unpredictable weather, a stormy night。The weather here is unpredictable—one minute sun, the next stormy. 构词：un- + predict + -able。

- **volcano / hike** — volcano 是火山，hike 是徒步。搭配：hike up a volcano, an active volcano。You can hike to the top of the volcano. 注意 volcano 的复数是 volcanoes（也可写 volcanos）。

- **tropical** — 热带的。搭配：a tropical island, tropical weather, tropical storm。构词：tropics（热带地区）→ tropical（形容词）。I dream of a tropical getaway.

- **not only...than / more...than** — not only 表"不仅"，常与 but also 搭配：Not only is it cheap, but it's also beautiful. more...than 表比较：It's more spontaneous than planned. 注意 not only 放句首时主句要倒装。

- **diner / deck** — diner 是美式小餐馆（通常路边、复古风格），deck 是甲板或露台。搭配：eat at a diner, sit on the deck。We stopped at a diner on the outskirts. / Let's sit on the deck and watch the sunset.

- **outskirts** — 郊区、市郊。搭配：on the outskirts of town, the outskirts。The hotel is on the outskirts. 注意 outskirts 通常用复数，搭配介词 on（不是 in）。

- **sunburn** — 晒伤。搭配：get a sunburn, sunburn cream。I got a bad sunburn on the tropical beach. 注意 sunburn 可作名词也可作动词：I got sunburned. / I have a sunburn.

## 练习

<quiz-pronunciation-match>

</quiz-pronunciation-match>

<quiz-pronunciation-spell>

</quiz-pronunciation-spell>

<quiz-translation-choice>

</quiz-translation-choice>

<quiz-sentence-builder>

1. **中文**：出国旅行时一定要准备一份备用计划。  
   **英文**：Always prepare a backup plan when traveling abroad.
   > **解析**：backup plan 表示备用计划。

2. **中文**：离开酒店前检查一下你的每日行程安排。  
   **英文**：Check your daily itinerary before leaving the hotel.
   > **解析**：itinerary 表示行程安排。

3. **中文**：我们在偏僻荒凉的地方发现了一个安静的小村庄。  
   **英文**：We found a small quiet village in the middle of nowhere.
   > **解析**：in the middle of nowhere 表示偏僻的地方。

4. **中文**：随身带把伞，以防万一下雨。  
   **英文**：Bring an umbrella with you just in case it rains.
   > **解析**：just in case 表示以防万一。

5. **中文**：该度假村位于城市的郊区外围。  
   **英文**：The resort is located on the outskirts of the city.
   > **解析**：outskirts 表示郊区、市郊。

6. **中文**：这个小岛位于意大利海岸附近海域。  
   **英文**：The small island lies off the coast of Italy.
   > **解析**：off the coast of 表示在……海岸附近的切向海域。

7. **中文**：节日周末的所有机票都已售罄。  
   **英文**：All plane tickets for the holiday weekend are sold out.
   > **解析**：sold out 表示售罄。

8. **中文**：我们临时起意决定周末驾车旅行。  
   **英文**：We made a spontaneous decision to take a weekend road trip.
   > **解析**：spontaneous 表示自发的、即兴的。

9. **中文**：我需要离开城市几天出去放松一下。  
   **英文**：I need to get away from the city for a few days.
   > **解析**：get away 表示外出度假、离开。

10. **中文**：我们计划明天沿着绿色的山径去徒步旅行。  
   **英文**：We plan to hike up the green mountain trail tomorrow.
   > **解析**：hike 表示徒步旅行。

11. **中文**：我们停在一家舒适的路边小餐馆吃午饭。  
   **英文**：We stopped at a cozy roadside diner for lunch.
   > **解析**：diner 表示路边小餐馆。

12. **中文**：在游船甲板上放松的同时欣赏海景。  
   **英文**：Enjoy the ocean view while relaxing on the ship deck.
   > **解析**：deck 表示甲板。

13. **中文**：航班因暴风雨天气条件而延误。  
   **英文**：The flight was delayed due to stormy weather conditions.
   > **解析**：stormy 表示暴风雨的。

14. **中文**：热带天气可能会非常炎热且不可预测。  
   **英文**：Tropical weather can be very warm and unpredictable.
   > **解析**：tropical 表示热带的。

15. **中文**：春季的高山天气往往不可预测。  
   **英文**：Mountain weather is often unpredictable during spring.
   > **解析**：unpredictable 表示不可预测的。

16. **中文**：涂抹防晒霜以避免被晒伤引发疼痛。  
   **英文**：Put on sunscreen to avoid getting a painful sunburn.
   > **解析**：sunburn 表示晒伤。

17. **中文**：活火山喷发吸引了许多游客。  
   **英文**：The active volcano erupted and drew many tourists.
   > **解析**：volcano 表示火山。

18. **中文**：这次假期不仅价格实惠，而且令人难忘。  
   **英文**：This vacation was not only affordable but also unforgettable.
   > **解析**：not only... but also... 表示不仅……而且……

19. **中文**：探索新地方比留在家中更有趣。  
   **英文**：Exploring new places is more exciting than staying home.
   > **解析**：more... than... 表示比……更……

20. **中文**：我们在纪念品商店稍作停留吧。  
   **英文**：Let's make a short stop at the souvenir shop.
   > **解析**：stop 表示停留、中途停靠。

</quiz-sentence-builder>

<quiz-listening>

1. Always prepare a backup plan when traveling abroad.
   > **中文**：出国旅行时一定要准备一份备用计划。

2. Check your daily itinerary before leaving the hotel.
   > **中文**：离开酒店前检查一下你的每日行程安排。

3. We found a small quiet village in the middle of nowhere.
   > **中文**：我们在偏僻荒凉的地方发现了一个安静的小村庄。

4. Bring an umbrella with you just in case it rains.
   > **中文**：随身带把伞，以防万一下雨。

5. The resort is located on the outskirts of the city.
   > **中文**：该度假村位于城市的郊区外围。

6. The small island lies off the coast of Italy.
   > **中文**：这个小岛位于意大利海岸附近海域。

7. All plane tickets for the holiday weekend are sold out.
   > **中文**：节日周末的所有机票都已售罄。

8. We made a spontaneous decision to take a weekend road trip.
   > **中文**：我们临时起意决定周末驾车旅行。

9. I need to get away from the city for a few days.
   > **中文**：我需要离开城市几天出去放松一下。

10. We plan to hike up the green mountain trail tomorrow.
   > **中文**：我们计划明天沿着绿色的山径去徒步旅行。

11. We stopped at a cozy roadside diner for lunch.
   > **中文**：我们停在一家舒适的路边小餐馆吃午饭。

12. Enjoy the ocean view while relaxing on the ship deck.
   > **中文**：在游船甲板上放松的同时欣赏海景。

13. The flight was delayed due to stormy weather conditions.
   > **中文**：航班因暴风雨天气条件而延误。

14. Tropical weather can be very warm and unpredictable.
   > **中文**：热带天气可能会非常炎热且不可预测。

15. Mountain weather is often unpredictable during spring.
   > **中文**：春季的高山天气往往不可预测。

16. Put on sunscreen to avoid getting a painful sunburn.
   > **中文**：涂抹防晒霜以避免被晒伤引发疼痛。

17. The active volcano erupted and drew many tourists.
   > **中文**：活火山喷发吸引了许多游客。

18. This vacation was not only affordable but also unforgettable.
   > **中文**：这次假期不仅价格实惠，而且令人难忘。

19. Exploring new places is more exciting than staying home.
   > **中文**：探索新地方比留在家中更有趣。

20. Let's make a short stop at the souvenir shop.
   > **中文**：我们在纪念品商店稍作停留吧。

</quiz-listening>

<quiz-matching>

| 英文单词 | 课文释义 |
| :--- | :--- |
| `backup plan` | 备用计划 |
| `deck` | 甲板 |
| `diner` | 路边小餐馆 |
| `get away` | 外出度假；离开 |
| `hike` | 徒步旅行 |
| `in the middle of nowhere` | 偏僻荒凉之地 |
| `itinerary` | 行程安排 |
| `just in case` | 以防万一 |
| `not only` | 不仅 |
| `off the coast of` | 在……海岸附近的切向海域 |
| `outskirts` | 郊区；市郊 |
| `sold out` | 售罄 |
| `spontaneous` | 即兴的；自发的 |
| `stop` | 停留；中途停靠 |
| `stormy` | 暴风雨的 |
| `sunburn` | 晒伤 |
| `tropical` | 热带的 |
| `unpredictable` | 不可预测的 |
| `volcano` | 火山 |
| `more` | 更 |

</quiz-matching>

<quiz-cloze>

### 1

Always prepare a backup plan when ____ abroad.

- [x] traveling
- [ ] backup plan
- [ ] deck

> **解析**：**traveling** 为动词的现在分词/动名词形式，配合句中语法结构。

### 2

Check your daily ____ before leaving the hotel.

- [x] itinerary
- [ ] backup plan
- [ ] deck

> **解析**：**itinerary** 符合句意表达，作为句中的核心词汇补全句子。

### 3

We found a small quiet ____ in the middle of nowhere.

- [x] village
- [ ] backup plan
- [ ] deck

> **解析**：**village** 符合句意表达，作为句中的核心词汇补全句子。

### 4

Bring an ____ with you just in case it rains.

- [x] umbrella
- [ ] backup plan
- [ ] deck

> **解析**：**umbrella** 符合句意表达，作为句中的核心词汇补全句子。

### 5

The resort is located on the ____ of the city.

- [x] outskirts
- [ ] backup plan
- [ ] deck

> **解析**：主语为第三人称单数，谓语动词使用单三形式 **outskirts**。

### 6

The small ____ lies off the coast of Italy.

- [x] island
- [ ] backup plan
- [ ] deck

> **解析**：**island** 符合句意表达，作为句中的核心词汇补全句子。

### 7

All plane ____ for the holiday weekend are sold out.

- [x] tickets
- [ ] backup plan
- [ ] deck

> **解析**：主语为第三人称单数，谓语动词使用单三形式 **tickets**。

### 8

We made a ____ decision to take a weekend road trip.

- [x] spontaneous
- [ ] backup plan
- [ ] deck

> **解析**：主语为第三人称单数，谓语动词使用单三形式 **spontaneous**。

### 9

I ____ to get away from the city for a few days.

- [x] need
- [ ] backup plan
- [ ] deck

> **解析**：**need** 符合句意表达，作为句中的核心词汇补全句子。

### 10

We plan to ____ up the green mountain trail tomorrow.

- [x] hike
- [ ] backup plan
- [ ] deck

> **解析**：**hike** 符合句意表达，作为句中的核心词汇补全句子。

### 11

We stopped at a cozy roadside ____ for lunch.

- [x] diner
- [ ] backup plan
- [ ] deck

> **解析**：**diner** 为形容词/副词比较级，用于两者之间的对比。

### 12

Enjoy the ocean view while relaxing on the ship ____.

- [x] deck
- [ ] backup plan
- [ ] diner

> **解析**：**deck** 符合句意表达，作为句中的核心词汇补全句子。

### 13

The flight was delayed due to ____ weather conditions.

- [x] stormy
- [ ] backup plan
- [ ] deck

> **解析**：**stormy** 符合句意表达，作为句中的核心词汇补全句子。

### 14

____ weather can be very warm and unpredictable.

- [x] Tropical
- [ ] backup plan
- [ ] deck

> **解析**：**Tropical** 符合句意表达，作为句中的核心词汇补全句子。

### 15

Mountain weather is often ____ during spring.

- [x] unpredictable
- [ ] backup plan
- [ ] deck

> **解析**：**unpredictable** 符合句意表达，作为句中的核心词汇补全句子。

### 16

Put on sunscreen to avoid getting a painful ____.

- [x] sunburn
- [ ] backup plan
- [ ] deck

> **解析**：**sunburn** 符合句意表达，作为句中的核心词汇补全句子。

### 17

The active ____ erupted and drew many tourists.

- [x] volcano
- [ ] backup plan
- [ ] deck

> **解析**：**volcano** 符合句意表达，作为句中的核心词汇补全句子。

### 18

This vacation was not only affordable but also ____.

- [x] unforgettable
- [ ] backup plan
- [ ] deck

> **解析**：**unforgettable** 符合句意表达，作为句中的核心词汇补全句子。

### 19

Exploring new places is ____ exciting than staying home.

- [x] more
- [ ] backup plan
- [ ] deck

> **解析**：**more** 符合句意表达，作为句中的核心词汇补全句子。

### 20

Let's make a short ____ at the souvenir shop.

- [x] stop
- [ ] backup plan
- [ ] deck

> **解析**：**stop** 符合句意表达，作为句中的核心词汇补全句子。

</quiz-cloze>