...
- 阅读器使用独立的标签四态辅助：默认纯阅读、仅看释义、排除和黄色 Tag，可组合表达 `GRE - 高考` 等范围
- 点击词条展示音标、中文释义、词性与词形变化
- 阅读辅助与词典浏览筛选相互独立；词典浏览的标注态在列表末尾显示等宽 Tag，纯阅读中的英文仍可点击查词
- 设置页汇总阅读器、词典、语义网络、扩展模块与多邻国进度，可分项或统一清空且保留数据缓存与使用偏好
- 支持段落/章节 TTS 朗读和人声跟读录音
- 朗读者选择由词典、阅读器和练习模块共享并持久保存；当前设备缺少原语音时自动回退到可用英文语音
...
src/
  ...
  components/
    TagSwitcher.vue              # 词典模块四态标签筛选组件
    ReaderAnnotationControls.vue # 阅读器独立四态辅助控制
